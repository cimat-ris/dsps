...
def get_fibermap(all_trajs, n):
    n_trajs       = all_trajs.shape[0]
    print(n_trajs)
    fiber_map     = np.zeros([2 * n, 2 * n, 3])  # define empty map for each streamline
    all_fibre_map = np.zeros([n_trajs, 2 * n, 2 * n, 3])  # to store all maps
    ...
    for j in range(n_trajs):
        ...
        for i in range(3):  # for each dimension in streamline
            ...
            block1     = np.expand_dims(np.concatenate((stream, stream_rev), axis=0),axis=0)  # build blocks
            block2     = np.expand_dims(np.concatenate((stream_rev, stream), axis=0),axis=0)
